...
const ContractCalculator: React.FC<ContractCalculatorProps> = ({
		...
		<div className="p-4">
			...
			<div className="mb-4">
				<div className="flex items-center gap-2 mb-2">
					<span id="sixStarLabel" className="font-bold">
						星6オペレーターの使用人数
					</span>
					<p className="text-gray-500 text-xs">一人減ると+3点</p>
				</div>
				<fieldset className="flex flex-wrap gap-2 max-w-[360px] border-0 p-0 m-0">
					<legend className="sr-only">星6オペレーターの使用人数</legend>
					{Array.from({ length: 14 }, (_, i) => (
						<button
							type="button"
							// biome-ignore lint/suspicious/noArrayIndexKey: 14個のボタンを作るため、indexをkeyに使用
							key={`sixstar-${i}`}
			...
			<div className="mb-4">
				<div className="flex items-center gap-2 mb-2">
					<span id="teamSizeLabel" className="font-bold">
						編成人数
					</span>
					<p className="text-gray-500 text-xs">一人減ると+5点</p>
				</div>
				<fieldset className="flex flex-wrap gap-2 max-w-[360px] border-0 p-0 m-0">
					<legend className="sr-only">編成人数</legend>
					{Array.from({ length: 14 }, (_, i) => (
						<button
							type="button"
							// biome-ignore lint/suspicious/noArrayIndexKey: 14個のボタンを作るため、indexをkeyに使用
